...
class EEGEncoder(nn.Module):
    ...
    def __init__(self, args, device=torch.cuda.set_device(0)): # torch.device('cuda:0')
        ...
        self.time_transformer_enc = TransformerEncoder(self.time_steps, self.feature_dim, self.hidden_dim,  nheads=3, depth=2, p=0.5, max_len=self.feature_dim)
        self.time_linear = nn.Sequential(nn.Dropout(self.dropout), nn.Linear(self.feature_dim, self.enc_dim, nn.LeakyReLU()))

        self.channel_transformer_enc = TransformerEncoder(self.channels, self.feature_len, self.hidden_dim, nheads=3,depth=2, p=0.5, max_len=self.feature_len)
        ...
        self.convlayer = nn.Conv1d(in_channels=self.labelNum,out_channels = self.labelNum, kernel_size = 3, padding=0)
