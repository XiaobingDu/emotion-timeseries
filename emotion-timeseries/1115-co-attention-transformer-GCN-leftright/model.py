...
class EEGEncoder(nn.Module):
    ...
    def __init__(self, args, device=torch.cuda.set_device(0)): # torch.device('cuda:0')
        ...
        self.feature_dim = args['feature_dim'] #300
        self.out_layer = args['out_layer']
        self.sequence_len = args['sequence_len'] # 60 timesteps
    ...
    def forward(self, x, left_features, right_features, target=None, tgt_init=0.0):
        ...
        # all_transformer
        left_features = torch.reshape(left_features,[left_features.shape[0],left_features.shape[1], left_features.shape[2]/5, 5])
        ...
        print('left_feature shape:', left_features.shape)
        right_features = torch.reshape(right_features, [right_features.shape[0], right_features.shape[1], right_features.shape[2]/5, 5])
